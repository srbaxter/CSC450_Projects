...
public class ChatClientAgent extends Agent implements ChatClientInterface {
	...
	public void notifySpoken(String speaker, String sentence) {
		...
		broadcast.putExtra("chat", speaker + ": " + sentence + "\n");
		logger.log(Level.INFO, "Sending broadcast " + broadcast.getAction());
		context.sendBroadcast(broadcast);
	}
				...
				if (msg.getPerformative() == ACLMessage.INFORM) {
					try {
						...
						if(p instanceof Joined) {
							...
							for(AID a : aid) {
                                participants.add(a);
                                System.out.println("NEW PART: " + participants + " | " + participants.toString());
                                Intent broadcast = new Intent();
                                broadcast.setAction("jade.demo.chat.REFRESH_CHAT");
                                broadcast.putExtra("sentence",a.getLocalName() + " has just entered the chat room!\n");
                                logger.log(Level.INFO, "Sending broadcast " + broadcast.getAction());
                                context.sendBroadcast(broadcast);
                            }
						...
						if(p instanceof Left) {
							...
							for(AID a : aid) {
                                ...
                                Intent broadcast = new Intent();
                                broadcast.setAction("jade.demo.chat.REFRESH_CHAT");
                                broadcast.putExtra("sentence",a.getLocalName() + " has just left!\n");
                                logger.log(Level.INFO, "Sending broadcast " + broadcast.getAction());
                                context.sendBroadcast(broadcast);
                            }
							notifyParticipantsChanged();
	 ...
	 * sentence
	 */
	public class ChatSpeaker extends OneShotBehaviour {
